...
- **Prepare** — media library, public URLs, music playlists, weather/YouTube plates
...
- **Playback** — YouTube/Vimeo URLs, HLS, MP4, images, slides, playlist carousel rotation, background music on visual items
...
1. Load content from uploads or public URLs.
...
| `/admin`                | Dashboard                                               |
| `/admin/prepare`        | Content hub (links to plates, media, URLs and music)    |
...
1. Open **Prepare** and add content: upload media, add YouTube/Vimeo/direct media URLs, create weather/YouTube plates, configure music playlists.
...
5. Open **Health** and fix any failing checks (env, D1, R2, output token, screens configured).
...
Cloudflare deploys must keep dashboard vars/secrets configured: `APP_ENCRYPTION_KEY`, `ADMIN_BOOTSTRAP_TOKEN`, `OUTPUT_CAPTURE_TOKEN`, and app base URLs. Deploy scripts use `--keep-vars` to preserve dashboard variables.
